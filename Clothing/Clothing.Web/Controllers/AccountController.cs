...
namespace Clothing.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            var user = repository.Customers.SingleOrDefault(u => u.Email == model.Email);
            if (user != null)
            {
                ViewBag.Notice = "User with that email already exists";
                return View("Register");
            }

            var passwordAndSalt = new Password(model.Password);
            user = new Customer
            {
        ...
        public void SetLoginCookies(Customer user)
        {
            var userCookie = new HttpCookie("svahidi_user", Server.UrlEncode(user.Email));
            userCookie.Expires.AddDays(365);
            HttpContext.Response.Cookies.Add(userCookie);
        }

        private bool VerifyUserLogin(string email, string password, out Customer user)
        {

            user = repository.Customers.SingleOrDefault(x => x.Email == email);

            if (user != null)
            {
                return Password.Verify(user.Salt, user.Password, password);
            }
            return false;
